...
namespace MornLib
{
	internal sealed class MornTransitionMaterial : MornTransitionBase
	{
		...
		public override async UniTask FillAsync(CancellationToken ct = default)
		{
			...
			await ColorTweenTask(start, 1, _fillDuration * (1 - start), _fillEase, _cts.Token);
		}
		...
		public override async UniTask ClearAsync(CancellationToken ct = default)
		{
			...
			await ColorTweenTask(start, 0, _clearDuration * start, _clearEase, _cts.Token);
		}
		...
		private async UniTask ColorTweenTask(float start, float end, float duration, MornEaseType ease, CancellationToken ct = default)
		{
			if (duration > 0)
			{
				var elapsedTime = 0f;
				while (elapsedTime < duration)
				{
					...
					var easedT = Mathf.Clamp01(elapsedTime / duration).Ease(ease);
					_material.SetFloat(_rateId, Mathf.Lerp(start, end, easedT));
